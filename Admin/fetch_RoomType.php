...
{
  $query .= '
  WHERE roomtype LIKE "%'.str_replace(' ', '%', $_POST['query']).'%" 
  ';
}
...
if($total_data > 0)
{
  foreach($result as $row)
  {
    ...
    <tr>
      ...
      <td class="d-none roomtype_id">'.$row["roomtypeid"].'</td>
      <td class="py-2" style="width: 300px;" >'.$row["roomtype"].'</td>

      <td class="py-2">'.$row["roomtypedescription"].'</td>
      <td class="py-2" style="width: 200px;"><span>&#8369 </span>'.$row["roomprice"].'.00</td>
      <td class="d-none">'.$row["roomprice"].'</td>
